...
System.register("template", [], function(exports_4, context_4) {
    ...
    return {
        setters:[],
        execute: function() {
            exports_4("DEFAULT_TEMPLATE", DEFAULT_TEMPLATE = "<pagination-template #p=\"paginationApi\" [id]=\"id\" [maxSize]=\"maxSize\" (pageChange)=\"pageChange.emit($event)\">	<div class=\"dataTables_paginate paging_simple_numbers\">		<ul class=\"pagination pagination-separated\" role=\"navigation\" aria-label=\"Pagination\" *ngIf=\"!(autoHide && p.pages.length <= 1)\">			<li class=\"paginate_button previous\" [class.disabled]=\"p.isFirstPage()\" *ngIf=\"directionLinks\"> <a (click)=\"p.previous()\" aria-label=\"Next page\">                Previous</a>			</li>			<li class=\"paginate_button\" [class.active]=\"p.getCurrent() === page.value\" *ngFor=\"let page of p.pages\">				<a (click)=\"p.setCurrent(page.value)\">{{ page.label }}</a>			</li>			<li class=\"paginate_button next\" [class.disabled]=\"p.isLastPage()\" *ngIf=\"directionLinks\"> <a  (click)=\"p.next()\" aria-label=\"Next page\">Next</a>			</li>		</ul>	</div></pagination-template>");
