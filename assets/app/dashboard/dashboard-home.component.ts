...
export class DashboardHomeComponent { 
	...
	@ViewChild('elementSearchComponent') elementSearchComponent: ElementSearchComponent;
	...
	elementSelected($event) {
		...
		if ($event.item && $event.item.hasattributes) {
			...
			this.dataService.findElementAttributesById(this.selectedElement.id.toLowerCase()).subscribe(
				event2 => this.gotAttributes($event, event2),
				error => this.handleError(error)
			);	
		}
	...
	gotAttributes(elementEvent, attrsEvent) {
	...
	displayChart(attr) {
		...
		this.dataService.getAttributeData(attr).subscribe(
			data => this.hasChartData(data),
			error => this.handleError(error)
		);
	}
